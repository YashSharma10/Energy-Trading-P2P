...
const getAIRecommendedPrice = async (listing, metrics, isProduct = false) => {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      const basePrice = isProduct ? listing.price : listing.pricePerCredit;
      const projectType = isProduct ? listing.category : listing.projectType;
      console.log(`[AI Pricing] Calculating dynamic price for ${isProduct ? "product" : "listing"}: ${listing._id}, Base Price: ₹${basePrice}`);
...
Return ONLY the JSON, no other text.`;
    ...
    if (!responseText) {
      ...
      console.warn("Gemini Response:", JSON.stringify(response.data, null, 2));
      return basePrice;
    }

    console.log(`[AI Pricing] Gemini Response Text: ${responseText.substring(0, 200)}...`);
    ...
    console.log(`[AI Pricing] Parsed Result - Recommended Price: ₹${result.recommendedPrice}, Multiplier: ${result.priceMultiplier}, Temp: ${result.marketTemperature}`);
    return {
      ...
      if (error.response?.status === 429 && attempt < MAX_RETRIES - 1) {
        console.warn(`[AI Pricing] Rate limited. Retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue;
      }
      if (attempt === MAX_RETRIES - 1) {
        console.error(`[AI Pricing] Max retries exceeded for listing ${listing._id}:`, error.message);
        break;
      }
    }
  }
  ...
  console.warn(`[AI Pricing] Falling back to base price for listing ${listing._id}`);
  return listing[isProduct ? "price" : "pricePerCredit"];
};
...
export const calculateDynamicPrice = async (listingId, isProduct = false) => {
  try {
    ...
    const listing = await model.findById(listingId).populate("seller", "ratings");
    ...
    const recommendedPrice = typeof aiRecommendation === 'number' 
      ? aiRecommendation 
      : aiRecommendation.recommendedPrice;
    
    const aiData = typeof aiRecommendation === 'number'
      ? {
          recommendedPrice,
          priceMultiplier: 1.0,
    ...
    console.log(`[AI Pricing] Saved Dynamic Price - Base: ₹${basePrice}, Recommended: ₹${aiData.recommendedPrice}, Savings: ₹${basePrice - aiData.recommendedPrice}`);
...
export const updateAllDynamicPrices = async () => {
  try {
    const listings = await CarbonCredit.find({ status: "Available" }).select("_id");
